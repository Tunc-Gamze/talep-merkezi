...
test('SQLite API: authentication, yetki, kullanıcı yönetimi ve müşteri akışı',async()=>{
 ...
 try{
  ...
  assert.equal((await call('PATCH','/api/notifications/read-all',{})).status,401);
  ...
  const firstNotifications=await call('GET','/api/notifications',undefined,supportCookie);
  assert.equal(firstNotifications.status,200);
  assert.equal(firstNotifications.data.unread_count,1);
  assert.equal(firstNotifications.data.notifications[0].type,'new_request');
  assert.equal(firstNotifications.data.notifications[0].request_number,number);
  assert.match(firstNotifications.data.notifications[0].title,/ABC/);
  await call('GET','/api/notifications',undefined,supportCookie);
  await call('GET','/api/notifications',undefined,supportCookie);
  assert.equal(db.prepare("SELECT COUNT(*) count FROM notifications WHERE user_id=? AND source_key LIKE 'request:%'").get(memberId).count,1);
  const firstNotificationId=firstNotifications.data.notifications[0].id;
  assert.equal((await call('PATCH','/api/notifications/'+firstNotificationId+'/read',{},secondCookie)).status,404);
  assert.equal((await call('PATCH','/api/notifications/'+firstNotificationId+'/read',{},supportCookie)).status,200);
  assert.equal((await call('GET','/api/notifications',undefined,supportCookie)).data.unread_count,0);
  ...
  const teamNoteNotifications=await call('GET','/api/notifications',undefined,supportCookie);
  assert.equal(teamNoteNotifications.data.unread_count,1);
  assert.equal(teamNoteNotifications.data.notifications[0].type,'team_note');
  assert.match(teamNoteNotifications.data.notifications[0].title,/Gamze Destek/);
  assert.equal((await call('GET','/api/notifications',undefined,secondCookie)).data.notifications.filter(notification=>notification.type==='team_note').length,0);
  assert.equal((await call('PATCH','/api/notifications/read-all',{},supportCookie)).status,200);
  ...
  const customerNotifications=await call('GET','/api/customer/notifications?phone=05321112233');
  assert.equal(customerNotifications.status,200);
  assert.equal(customerNotifications.data.unread_count,3);
  assert.ok(customerNotifications.data.notifications.some(notification=>notification.type==='team_message'&&notification.message.includes('Hazır')));
  assert.equal(customerNotifications.data.notifications.filter(notification=>notification.type==='status_changed').length,2);
  assert.equal(customerNotifications.data.notifications[0].public_token,token);
  assert.equal((await call('GET','/api/customer/notifications?phone=05329999999')).data.notifications.length,0);
  const customerNotificationId=customerNotifications.data.notifications[0].id;
  assert.equal((await call('PATCH','/api/customer/notifications/'+customerNotificationId+'/read',{phone:'05329999999'})).status,404);
  assert.equal((await call('PATCH','/api/customer/notifications/'+customerNotificationId+'/read',{phone:'05321112233'})).status,200);
  assert.equal((await call('GET','/api/customer/notifications?phone=05321112233')).data.unread_count,2);
  assert.equal((await call('PATCH','/api/customer/notifications/read-all',{phone:'05321112233'})).status,200);
  assert.equal((await call('GET','/api/customer/notifications?phone=05321112233')).data.unread_count,0);
  assert.equal((await call('POST','/api/public/requests/'+number,{token,message:'Teşekkürler'})).status,201);
  const messageNotifications=await call('GET','/api/notifications',undefined,supportCookie);
  assert.equal(messageNotifications.data.unread_count,1);
  assert.equal(messageNotifications.data.notifications[0].type,'customer_message');
  assert.equal(messageNotifications.data.notifications[0].message,'Teşekkürler');
  assert.equal(db.prepare("SELECT COUNT(*) count FROM notifications WHERE user_id=? AND type='customer_message'").get(memberId).count,1);
  assert.equal((await call('PATCH','/api/notifications/read-all',{},supportCookie)).status,200);
  assert.equal((await call('GET','/api/notifications',undefined,supportCookie)).data.unread_count,0);
  assert.ok((await call('GET','/api/notifications',undefined,supportCookie)).data.notifications.every(notification=>notification.read_at));
  assert.equal((await call('GET','/api/notifications',undefined,secondCookie)).data.unread_count,2);
